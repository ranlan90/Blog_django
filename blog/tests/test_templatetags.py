...
from ..templatetags.blog_extras import show_archives,show_category,show_recent_post,show_tag
...
class BlogExtrasTestCase(TestCase):
    ...
    def test_show_recent_post_without_any_post(self):
        template = Template("{% load blog_extras %}" "{% show_recent_post %}")
    ...
    def test_show_recent_post_with_posts(self):
        post = Post.objects.create(
            title="测试标题", body="测试内容", category=self.cate, author=self.user,
        )
        context = Context(show_recent_post(self.ctx))
        template = Template("{% load blog_extras %}" "{% show_recent_post %}")
    ...
    def test_show_recent_post_nums_specified(self):
        post_list = []
        for i in range(7):
            post = Post.objects.create(
                title="测试标题-{}".format(i),
        ...
        context = Context(show_recent_post(self.ctx, 3))
        template = Template("{% load blog_extras %}" "{% show_recent_post %}")
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">最新文章</h3>', expected_html)
        self.assertInHTML(
    ...
    def test_show_category_without_any_category(self):
        self.cate.delete()
        context = Context(show_category(self.ctx))
        template = Template("{% load blog_extras %}" "{% show_category %}")
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">分类</h3>', expected_html)
        self.assertInHTML("暂无分类！", expected_html)

    def test_show_category_with_categories(self):
    ...
    def test_show_tag_without_any_tag(self):
        context = Context(show_tag(self.ctx))
        template = Template("{% load blog_extras %}" "{% show_tag %}")
        expected_html = template.render(context)
        self.assertInHTML('<h3 class="widget-title">标签云</h3>', expected_html)
        self.assertInHTML("暂无标签！", expected_html)

    def test_show_tag_with_tags(self):
